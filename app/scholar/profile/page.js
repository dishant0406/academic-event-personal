...
export default function ScholarProfile() {
  ...
  const getInitials = (name) => name ? name.split(" ").map(n => n[0]).join("").substring(0, 2).toUpperCase() : "SC";
  ...
  return (
    <>
      <div className="dashboard-header">
        <h1>My Profile 👤</h1>
        <p>Manage your research scholar account</p>
      </div>
      <div className="form-container" style={{ maxWidth: "100%", position: "relative" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 24, marginBottom: 32, paddingBottom: 24, borderBottom: "1px solid var(--border)" }}>
          <div style={{ width: 80, height: 80, borderRadius: "50%", background: "linear-gradient(135deg, #8b5cf6, #d946ef)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "2rem", color: "white", fontWeight: "bold" }}>
            {getInitials(user.fullName)}
          </div>
          <div>
            <h3 style={{ fontFamily: "Space Grotesk, sans-serif", fontSize: "1.2rem", fontWeight: 700 }}>{user.fullName}</h3>
            <p style={{ color: "var(--text-secondary)", fontSize: "0.85rem", fontFamily: "Outfit, sans-serif" }}>Ph.D. Scholar · Dept. of {user.department}</p>
        ...
        <div className="form-row">
          <div className="form-group">
            ...
            <input className="form-input" value={form.researchDomain} onChange={(e) => setForm({...form, researchDomain: e.target.value})} />
          </div>
          <div className="form-group">
            <label className="form-label">Supervisor</label>
            <input className="form-input" value={form.supervisor} onChange={(e) => setForm({...form, supervisor: e.target.value})} />
          </div>
        </div>
        <div className="form-group">
        ...
        <button 
          ...
          style={{ width: "100%", justifyContent: "center", background: "#8b5cf6", border: "none" }}
          onClick={handleSave}
          disabled={saving}
        >
